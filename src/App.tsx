...
const STARTERS: Template[] = [
  ...
  {id:'style-bold-pop',name:'極太ポップライン',description:'極太の黒線と黄・シアン・ピンクの鮮烈な配色',icon:'Ｐ',accent:'#ffe500',category:'テイスト変換',thumbnail:import.meta.env.BASE_URL+'presets/bold-pop-reference.png',uses:0,updated:5,prompt:'1枚目の画像を変更対象として使用してください。1枚目のキャラクターや被写体の顔立ち、髪型、衣装、固有特徴は維持し、別人化させないでください。\n\n2枚目の画像はテイストの参考として使用してください。人物や衣装そのものはコピーせず、極太で均一な黒い輪郭線、平面的で明快な塗り、鮮やかな黄色・シアン・マゼンタ・黒を中心とした限定色、色版がずれたような縁取りとステッカー風のグラフィック表現を{styleStrength}取り入れてください。\n\n細かな写実表現や淡い陰影は避け、シルエットが一目で伝わる大胆でポップなイラストにしてください。背景は単純化し、被写体と配色のインパクトを最優先してください。\n{thumbnail}\n{composition}\n{custom}\n{ratio}\n{noText}'},
...
export function App(){
  ...
  useEffect(()=>{setTemplates(current=>{const glow=STARTERS.find(s=>s.id==='style-glow')!;const migrated=current.map(t=>t.id==='style-glow'?{...t,prompt:t.prompt.includes('{referenceUrl}')?glow.prompt:t.prompt,thumbnail:t.thumbnail||glow.thumbnail}:t);const missing=STARTERS.filter(s=>!migrated.some(t=>t.id===s.id));return missing.length?[...migrated,...missing]:migrated})},[]);
  ...
  const choose=(t:Template)=>{const isStyle=t.id==='style'||t.category==='テイスト変換';setMode(isStyle?'style':'prompt');setActiveId(t.id);if(isStyle)setStyle(t.thumbnail?{data:t.thumbnail,name:`${t.name}・参考画像`}:null);if(t.id==='style-glow'){setStyleParts(['光の美しさ','瞳のテイスト','色彩','線画']);setStyleStrength(3);setTextMode('none');setStyleRecompose(true);setThumbnailMode(true)}if(t.id==='style-bold-pop'){setStyleParts(['色彩','線画','塗りの質感']);setStyleStrength(3);setTextMode('none');setStyleRecompose(true);setThumbnailMode(true)}if(t.id==='style'){setStyleParts(['色使い','線や塗りの質感','ライティング','全体の雰囲気']);setStyleStrength(2);setTextMode('none')}if(t.id==='comic'){setTextMode('dialogue');if(!dialogue)setDialogue('！')}if(t.id==='cold-anger'&&dialogue==='！')setDialogue('');setLibraryOpen(false)};
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="mode-switch" role="tablist" aria-label="作成モード">
        ...
        <button role="tab" aria-selected={mode==='style'} className={mode==='style'?'active':''} onClick={()=>{setMode('style');setActiveId('style');setStyle(null);setStyleParts(['色使い','線や塗りの質感','ライティング','全体の雰囲気']);setStyleStrength(2);setTextMode('none');setStyleRecompose(true);setThumbnailMode(false)}}><ArrowLeftRight size={17}/><span><b>2枚でテイスト変換</b><small>別画像の色・質感を参考にする</small></span></button>
      </div>
