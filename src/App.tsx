...
export function App(){
  ...
  const replacements:Record<string,string>={
    dialogue:dialogue||'！',custom:custom||'自由な発想で魅力的に仕上げてください。',framing,expression,
  ...
  const choose=(t:Template)=>{setMode(t.id==='style'?'style':'prompt');setActiveId(t.id);if(t.id==='comic')setTextMode('dialogue');setLibraryOpen(false)};
  async function copyPrompt(){await navigator.clipboard.writeText(prompt);setTemplates(v=>v.map(x=>x.id===active.id?{...x,uses:x.uses+1,updated:Date.now()}:x));setHistory(v=>[{id:uid(),prompt,template:active.name,at:Date.now()},...v].slice(0,50));setToast({text:'プロンプトをコピーしました',kind:'ok'})}
