...
const GLOW_REFERENCE_URL='https://alicetel-u.github.io/prompt-palette/presets/glow-eye-reference.png';
...
export function App(){
  ...
  const replacements:Record<string,string>={
    dialogue:dialogue||'（セリフ未入力）',referenceUrl:GLOW_REFERENCE_URL,custom:custom||'自由な発想で魅力的に仕上げてください。',framing,expression,
  ...
  const openChatGPT=async()=>{if(!source){setToast({text:'変更したい画像を追加してください'});return}await navigator.clipboard.writeText(prompt);window.open(`https://chatgpt.com/?q=${encodeURIComponent(prompt)}`,'_blank','noopener,noreferrer');setToast({text:'ChatGPTを開きました。画像1を添付してください',kind:'ok'})};
